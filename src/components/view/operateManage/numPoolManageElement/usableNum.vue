        ...
        <div class="search">
            <el-form ref="usableNumForm" :model="usableNumForm">
                <el-form-item style="float: left;margin-left: 15px;">
                    ...
                    <el-input
                        v-model="usableNumForm.number"
                        placeholder="请输入400号码"
                        size="mini"
                        style="width:300px;"
                    ></el-input>
                </el-form-item>
                ...
                <el-form-item label="可见渠道：" style=" clear: both;margin-left: 15px;">
                    <el-checkbox-group
                        v-model="usableNumForm.checkList"
                ...
                <div>
                    <el-form ref="addNumberForm" :model="addNumberForm" label-width="100px">
                        ...
                        <el-form-item label="可见渠道：">
                            <el-checkbox-group
                                v-model="addNumberForm.checkList"
                        ...
                        <el-form-item label="引示号码：">
                            <el-select
                                v-model="addNumberForm.guideNumberValue"
                                placeholder="请选择"
                                size="mini"
                                class="el-input"
                                @focus="getCanUseCitationNum"
                            >
                                <el-option
                        ...
                        <el-form-item label="关联套餐：" style="margin-top: 15px;">
                            <el-tabs v-model="addNumberForm.activeName" type="card">
                                <el-tab-pane label="自助直销" name="first">
                                    <el-checkbox-group
                                        v-model="addNumberForm.selfCheckList"
                                        v-for="(item,index) in this.addNumberForm.selfTableData.length"
                                        :key="item"
                                    >
                                        <el-checkbox
                                            :label="addNumberForm.selfTableData[index].tariffName"
                                        ></el-checkbox>
                                        <br>
                                    </el-checkbox-group>
                                </el-tab-pane>
                                <el-tab-pane label="渠道" name="second">
                                    <el-checkbox-group
                                        v-model="addNumberForm.channelCheckList"
                                        v-for="(item,index) in this.addNumberForm.channelTableData.length"
                                        :key="item"
                                    >
                                        <el-checkbox
                                            :label="addNumberForm.channelTableData[index].tariffName"
                                        ></el-checkbox>
                                        <br>
                                    </el-checkbox-group>
                                </el-tab-pane>
        ...
        <div>
            ...
            >
                <div>
                    <el-form ref="addNumberForm" :model="importNumberForm" label-width="150px">
                        <el-form-item label="校验比对结果：">
                            <span
                                style="float: left;"
                            >可导入号码{{importNumberForm.totalNum}}个，有{{importNumberForm.errorNum}}个号码有误</span>
                        </el-form-item>
                ...
                <span slot="footer" class="dialog-footer">
                    ...
                    <el-button
                        type="primary"
                        @click="importNumberFormDialogVisible = false"
                        size="mini"
                    >确 定</el-button>
                </span>
            </el-dialog>
    ...
    methods: {
                        ...
                        this.$ajax
                            ...
                            .then(resp => {
                                if (resp.code == 200) {
                                    for (
                                        let i = 0;
                                        i < resp.data.guideNumbers.length;
                                        i++
                                    ) {
                                        if (
        ...
        getIdsFunction() {
            ...
            for (let i = 0; i < this.addNumberForm.selfCheckList.length; i++) {
                for (
                    let j = 0;
                    j < this.addNumberForm.selfTableData.length;
                    j++
                ) {
                    if (
                        this.addNumberForm.selfCheckList[i] ==
                        this.addNumberForm.selfTableData[j].tariffName
                    ) {
            ...
            for (
                let i = 0;
                i < this.addNumberForm.channelCheckList.length;
                i++
            ) {
                for (
                    let j = 0;
                    j < this.addNumberForm.channelTableData.length;
                    j++
                ) {
                    if (
                        this.addNumberForm.channelCheckList[i] ==
                        this.addNumberForm.channelTableData[j].tariffName
                    ) {
