		...
		<div style="margin-top: 15px;">
			<el-collapse v-model="activeNames" @change="handleChange">
				<el-collapse-item title="自助直销" name="3">
					<div>
						<el-row>
							<el-col :md="11" :lg="7" :xl="5" v-for="(o, index) in bTotalCount1" :key="o" :offset="1" style="margin-top: 10px;">
				...
				<el-collapse-item title="渠道" name="4">
					<div>
						<el-row>
							<el-col :md="11" :lg="7" :xl="5" v-for="(o, index) in bTotalCount2" :key="o" :offset="1" style="margin-top: 10px;">
								<el-card :body-style="{ padding: '0px'}" shadow="hover">
										...
										<el-form label-width="100px">
											...
											<div style="border-top: 1px solid #f5f5f5;">
												<div style="float: right;">
													...
													<el-button type="text" class="button" :style="'display:'+bTableData2[index].type+';'" @click="deleteInfo(bTableData2[index].id,'delValueAdded')">删除</el-button>
												</div>
											</div>
										</el-form>
									</div>
					...
					<el-form ref="AddComboForm" :model="AddComboForm" label-width="100px">
						...
						<el-form-item label="计价单位：">
							<el-select v-model="AddComboForm.value" placeholder="请选择" style="float: left;width: 80%" size="mini">
								<el-option v-for="item in AddComboForm.options" :key="item.value" :label="item.label" :value="item.value"></el-option>
							</el-select>
						</el-form-item>
		...
		methods: {
			...
			addCombo() {
				if (
					...
					this.AddComboForm.radio == "" ||
					this.AddComboForm.type == "" ||
					this.AddComboForm.checkList.length == 0
				) {
					this.$message({
						message: "存在空字段!",
						type: "warning"
					...
					if (
						this.AddComboForm.checkList.length == 1 &&
						this.AddComboForm.checkList[0] == "自助直销"
					) {
						str = "self";
					}
					if (
						this.AddComboForm.checkList.length == 1 &&
						this.AddComboForm.checkList[0] == "渠道"
					) {
						str = "channel";
					}
					this.$ajax
						.post("/vos/tariffPackage/addValueAdded", {
							valueAdded: {
			...
			updateCombo() {
				if (
					...
					this.$ajax
						.post("/vos/tariffPackage/updateValueAdded", {
							valueAdded: {
