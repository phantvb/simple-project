		...
		<div style="margin-top: 15px;">
			<el-collapse v-model="activeNames" @change="handleChange">
				<el-collapse-item title="自助直销" name="7">
					<div>
						<el-row>
							<el-col :md="11" :lg="7" :xl="5" v-for="(o, index) in dTotalCount1" :key="o" :offset="1" style="margin-top: 10px;">
								<el-card :body-style="{ padding: '0px'}" shadow="hover">
									<div class="add-title">{{dTableData1[index].timePacketName}}</div>
									<div class="add-body">
										<el-form label-width="100px">
											<el-form-item label="充值额度：" class="add-item">{{dTableData1[index].rechargeLimit}}元</el-form-item>
				...
				<el-collapse-item title="渠道" name="8">
					<div>
						<el-row>
							<el-col :md="11" :lg="7" :xl="5" v-for="(o, index) in dTotalCount2" :key="o" :offset="1" style="margin-top: 10px;">
								<el-card :body-style="{ padding: '0px'}" shadow="hover">
									<div class="add-title">{{dTableData2[index].timePacketName}}</div>
									<div class="add-body">
										<el-form label-width="100px">
											<el-form-item label="充值额度：" class="add-item">{{dTableData2[index].rechargeLimit}}元</el-form-item>
											<el-form-item label="拨打时长：" class="add-item">{{dTableData2[index].dialingTime}}分钟</el-form-item>
											<div style="border-top: 1px solid #f5f5f5;">
												<div style="float: right;">
													<el-button type="text" class="button" @click="updateTimePacket(1,index)">编辑</el-button>
													<el-button type="text" class="button" @click="deleteInfo(dTableData2[index].id,'delTimePacket')">删除</el-button>
												</div>
											</div>
										</el-form>
									</div>
		...
		<div class="addTimePacketDialog">
			<el-dialog title="新增/编辑时长包方案" :visible.sync="addTimePacketFormDialogVisible" width="30%" :before-close="handleClose">
				<div>
					<el-form ref="addTimePacketForm" :model="addTimePacketForm" label-width="100px">
						<el-form-item label="时长包名：">
							<el-input v-model="addTimePacketForm.timePacketName" size="mini" class="el-input"></el-input>
						</el-form-item>
				...
				<span slot="footer" class="dialog-footer">
					...
					<el-button type="primary" @click="addTimePacket" size="mini" :style="'display:'+submitData+';'">确 定</el-button>
					<el-button type="primary" @click="handleUpdateTimePacket" size="mini" :style="'display:'+updateData+';'">编 辑</el-button>
				</span>
			</el-dialog>
		</div>
	</div>
		...
		methods: {
			...
			addTimePacket() {
				if (
					...
					this.addTimePacketForm.rechargeLimit == "" ||
					this.addTimePacketForm.dialingTime == "" ||
					this.addTimePacketForm.checkList.length == 0
				) {
					this.$message({
						message: "存在空字段!",
						type: "warning"
					...
					if (
						this.addTimePacketForm.checkList.length == 1 &&
						this.addTimePacketForm.checkList[0] == "自助直销"
					) {
						str = "self";
					}
					if (
						this.addTimePacketForm.checkList.length == 1 &&
						this.addTimePacketForm.checkList[0] == "渠道"
					) {
						str = "channel";
					}
					this.$ajax
						.post("/vos/tariffPackage/addTimePacket", {
							timePacket: {
								timePacketName: this.addTimePacketForm
									.timePacketName,
			...
			updateTimePacket(num, index) {
				...
				this.addTimePacketForm.timePacketName = this.tableData[num][
					index
				].timePacketName;
				this.addTimePacketForm.rechargeLimit = this.tableData[num][
					index
				].rechargeLimit;
				this.addTimePacketForm.dialingTime = this.tableData[num][
					index
				].dialingTime;
			...
			handleUpdateTimePacket() {
				if (
					...
					this.$ajax
						.post("/vos/tariffPackage/updateTimePacket", {
							timePacket: {
