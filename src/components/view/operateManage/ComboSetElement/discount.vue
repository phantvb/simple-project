		...
		<div style="margin-top: 15px;">
			<el-collapse v-model="activeNames" @change="handleChange">
				<el-collapse-item title="自助直销" name="5">
					<div>
						<el-row>
							<el-col :md="11" :lg="7" :xl="5" v-for="(o, index) in cTotalCount1" :key="index" :offset="1" style="margin-top: 10px;">
								<el-card :body-style="{ padding: '0px'}" shadow="hover">
									<div class="add-title">{{cTableData1[index].concessionName}}</div>
									<div class="add-body">
										<el-form label-width="100px">
											<el-form-item label="优惠方式：" class="add-item">{{cTableData1[index].concessionWay}}</el-form-item>
											<el-form-item label="充值额度：" class="add-item" v-if="cTableData1[index].rechargeLimit">{{cTableData1[index].rechargeLimit}}元</el-form-item>
											<el-form-item label="赠送额度：" class="add-item" v-if="cTableData1[index].donationQuota">{{cTableData1[index].donationQuota}}元</el-form-item>
											<el-form-item label="优惠价格：" class="add-item" v-if="cTableData1[index].packageDeals">{{cTableData1[index].packageDeals}}元</el-form-item>
											<el-form-item label="减扣时长：" class="add-item" v-if="cTableData1[index].availableTime">{{cTableData1[index].availableTime}}分钟</el-form-item>
											<el-form-item label="优惠时间：" class="add-item" v-if="cTableData1[index].concessionTime">{{cTableData1[index].concessionTime}} {{cTableData1[index].units}}</el-form-item>
				...
				<el-collapse-item title="渠道" name="6">
					<div>
						<el-row>
							<el-col :md="11" :lg="7" :xl="5" v-for="(o, index) in cTotalCount2" :key="index" :offset="1" style="margin-top: 10px;">
								<el-card :body-style="{ padding: '0px'}" shadow="hover">
									<div class="add-title">{{cTableData2[index].concessionName}}</div>
									<div class="add-body">
										<el-form label-width="100px">
											<el-form-item label="优惠方式：" class="add-item">{{cTableData2[index].concessionWay}}</el-form-item>
											<el-form-item label="充值额度：" class="add-item" v-if="cTableData2[index].rechargeLimit">{{cTableData2[index].rechargeLimit}}元</el-form-item>
											<el-form-item label="赠送额度：" class="add-item" v-if="cTableData2[index].donationQuota">{{cTableData2[index].donationQuota}}元</el-form-item>
											<el-form-item label="优惠价格：" class="add-item" v-if="cTableData2[index].packageDeals">{{cTableData2[index].packageDeals}}元</el-form-item>
											<el-form-item label="减扣时长：" class="add-item" v-if="cTableData2[index].availableTime">{{cTableData2[index].availableTime}}分钟</el-form-item>
											<el-form-item label="优惠时间：" class="add-item" v-if="cTableData2[index].concessionTime">{{cTableData2[index].concessionTime}} {{cTableData2[index].units}}</el-form-item>
											<div style="height:40px;" v-if="cTableData2[index].rechargeLimit"></div>
											<div style="border-top: 1px solid #f5f5f5;">
												<div style="float: right;">
													<el-button type="text" class="button" @click="updateDiscount(1,index)">编辑</el-button>
													<el-button type="text" class="button" @click="deleteInfo(cTableData2[index].id,'delConcessionScheme')">删除</el-button>
												</div>
											</div>
										</el-form>
									</div>
		...
		<div class="addDiscountDialog">
			<el-dialog title="新增优惠方案" :visible.sync="addDiscountFormDialogVisible" width="30%">
				<div>
					<el-form ref="addDiscountForm" :model="addDiscountForm" label-width="100px">
						<el-form-item label="优惠名称：">
							<el-input v-model="addDiscountForm.concessionName" size="mini" class="el-input"></el-input>
						</el-form-item>

						<el-form-item label="优惠方案：">
							<el-select v-model="addDiscountForm.value" placeholder="请选择" style="float: left;width: 80%" size="mini" @change="changeStyle">
								<el-option v-for="item in addDiscountForm.options" :key="item.value" :label="item.label" :value="item.value"></el-option>
							</el-select>
						</el-form-item>

						<el-form-item label="充值额度：" :style="'display:'+addDiscountForm.style1+';'">
							<el-input v-model="addDiscountForm.rechargeLimit" size="mini" class="el-input"></el-input>
							<span class="span">元</span>
						</el-form-item>

						<el-form-item label="赠送额度：" :style="'display:'+addDiscountForm.style1+';'">
							<el-input v-model="addDiscountForm.donationQuota" size="mini" class="el-input"></el-input>
							<span class="span">分钟</span>
						</el-form-item>

						<el-form-item label="优惠价格：" :style="'display:'+addDiscountForm.style2+';'">
							<el-input v-model="addDiscountForm.packageDeals" size="mini" class="el-input"></el-input>
							<span class="span">元</span>
						</el-form-item>

						<el-form-item label="减扣时长：" :style="'display:'+addDiscountForm.style2+';'">
							<el-input v-model="addDiscountForm.availableTime" size="mini" class="el-input"></el-input>
							<span class="span">分钟</span>
						</el-form-item>

						<el-form-item label="优惠时间：" :style="'display:'+addDiscountForm.style2+';'">
							<el-input v-model="addDiscountForm.concessionTime" size="mini" class="el-input" style="width: 40%;"></el-input>
							<el-select v-model="addDiscountForm.value2" placeholder="请选择" style="float: left; margin-left: 10%; width: 30%;" size="mini">
								<el-option v-for="item in addDiscountForm.options2" :key="item.value" :label="item.label" :value="item.value"></el-option>
							</el-select>
						</el-form-item>
				...
				<span slot="footer" class="dialog-footer">
					...
					<el-button type="primary" @click="addDiscount" size="mini" :style="'display:'+submitData+';'">确 定</el-button>
					<el-button type="primary" @click="handleUpdateDiscount" size="mini" :style="'display:'+updateData+';'">编 辑</el-button>
				</span>
			</el-dialog>
		</div>
	</div>
	...
	export default {
		...
		methods: {
			...
			updateDiscount(num, index) {
				...
				this.addDiscountForm.concessionName = this.tableData[num][index].concessionName;
				this.addDiscountForm.value = value;
				this.addDiscountForm.rechargeLimit = this.tableData[num][index].rechargeLimit;
				this.addDiscountForm.donationQuota = this.tableData[num][index].donationQuota;
				this.addDiscountForm.packageDeals = this.tableData[num][index].packageDeals;
				this.addDiscountForm.availableTime = this.tableData[num][index].availableTime;
				this.addDiscountForm.concessionTime = this.tableData[num][index].concessionTime;
