<template>
	<div id="selfHelp">
				...
				<el-form ref="addTopForm" :model="addTopForm" label-width="100px">
					...
					<div class="accountMessage">
						...
						<el-form-item label="归属地区：">
							{{detailsDialogForm.province}}{{detailsDialogForm.city}}{{detailsDialogForm.area}}
						</el-form-item>
			...
			<el-form ref="form" :model="form" label-width="100px">
				...
				<div>
					<el-form-item label="选择角色：">
						...
						<el-button size="mini" v-for="(item,index) in this.allRoleTableData" :key="index" @click="searchMess(allRoleTableData[index].name)">{{allRoleTableData[index].nameZh}}
						</el-button>
					</el-form-item>
				</div>
			</el-form>
		...
		<div class="accountTable">
			...
			<!--分页-->
			<el-pagination style="margin-top: 10px" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="page.currentPage" :page-sizes="$global.pageSize" :page-size="page.size" layout="total, sizes, prev, pager, next, jumper" :total="page.total">
			</el-pagination>
		</div>
		...
		methods: {
			...
			searchMess(roleName) {
				if (this.accountStatus == '1') {
					this.$ajax.post('/vos/user/searchUser2', {
						"user": {
				...
				if (this.accountStatus == '2') {
					this.$ajax.post('/vos/user/searchUser', {
						"user": {
							"username": this.form.accountNumber,
							"roleName": roleName,
							"name": this.form.userName,
			...
			loadCity() {
				...
				this.addTopForm.district = '';
				this.options.city = [];
				this.$ajax.get('/vos/address/getCitiesByProvinceId?provinceId=' + this.addTopForm.province).then(res => {
					if (res.code == 200) {
						for (let i = 0; i < res.data.length; i++) {
			...
			loadArea() {
				this.addTopForm.district = '';
				this.options.area = [];
				this.$ajax.get('/vos/address/getAreasByCityId?cityId=' + this.addTopForm.city).then(res => {
					if (res.code == 200) {
						for (let i = 0; i < res.data.length; i++) {
			...
			batchUpdatePass() {
				if (this.multipleSelection.length == 0) {
					...
					this.$confirm('此操作将批量重置密码, 是否继续?', '提示', {
						...
						this.$ajax.post('/vos/user/editPasswordAll', {
							"user": this.ids
			...
			batchStartAndStop(num) {
				if (this.multipleSelection.length == 0) {
					...
					this.$confirm('此操作是批量操作, 是否继续?', '提示', {
						...
						this.$ajax.post('/vos/user/setEnabled', {
							"user": this.ids
			...
			batchDelete() {
				if (this.multipleSelection.length == 0) {
					...
					this.$confirm('此操作将永久删除这些信息, 是否继续?', '提示', {
						...
						this.$ajax.post('/vos/user/delete', {
							"user": this.ids
			...
			loadTableData(strURL, num) {
				...
				this.$ajax.post('/vos/user/' + strURL, {
					"user": {
						"username": "",
						"roleName": "",
						"name": "",
