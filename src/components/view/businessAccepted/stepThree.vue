<template>
	<div id="stepThree">
		...
		<div class="addNumDialog">
			<el-dialog title="新增号码" :visible.sync="addNumdialogVisible" width="80%" append-to-body :before-close="handleClose">
				<div>
							...
							<el-form ref="addNumform" :model="addNumform" class="addNumform" label-width="120px">
								<div class="selectNumTable">
									<ul class="abc">
										<li style="float: unset">
											<el-table :data="selectedNum" border style="width: 100%">

												<el-table-column prop="number400" label="400号码">
												</el-table-column>

												<el-table-column prop="tariffName" label="套餐">
												</el-table-column>

												<el-table-column prop="basicFunctionFeeCopy" label="基本功能费">
												</el-table-column>

												<el-table-column prop="durationPresentationCopy" label="预存话费">
												</el-table-column>

												<el-table-column prop="unitsCopy" label="单位">
												</el-table-column>

												<el-table-column prop="packageContent" label="套餐详细">
												</el-table-column>

												<el-table-column label="操作">
													<template slot-scope="scope">
														<el-button size="mini" type="text" @click="delNum(scope.$index)">删除
														</el-button>
													</template>
												</el-table-column>
											</el-table>
								...
								<el-tabs type="border-card" @tab-click="parentMethod">
									<el-tab-pane v-for="(item,index) in mealList" :label="item.tariffName" :name="index+''" :key="index+''">
										<set-meal ref="child1" :unitMeal="item" :unitIndex="index" :getAllByPackages="getAllByPackage"></set-meal>
									</el-tab-pane>
								</el-tabs>
				...
				<div class="searchNumDialog">
					<el-dialog :title="titleNum+'的搜索结果'" :visible.sync="searchNumdialogVisible" width="80%" append-to-body :before-close="handleClose">
						<div>
							<el-table :data="searchNumList" border style="width: 100%">
								...
								<el-table-column prop="basicFunctionFee" label="基本功能费">
								</el-table-column>

								<el-table-column prop="durationPresentation" label="预存话费">
								</el-table-column>
		...
		<el-form ref="stepThreeForm" :model="stepThreeForm" :rules="rules" label-width="120px">
				...
				<div>
					...
					<ul class="abc">
						<li class="l2">
							<ul class="abc">
								<li class="l2">
									<el-upload class="avatar-uploader exampleh" :action="$global.uploadUrl" :show-file-list="false" :on-success="handleAvatarSuccess" :on-error="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
										<img v-if="stepThreeForm.agentCardFront" :src="stepThreeForm.agentCardFront" class="avatar">
										<i v-else class="el-icon-plus avatar-uploader-icon"></i>
									</el-upload>
								</li>
								<li class="l2">
									<el-upload class="avatar-uploader exampleh" :action="$global.uploadUrl" :show-file-list="false" :on-success="handleBackSuccess" :on-error="handleBackSuccess" :before-upload="beforeAvatarUpload">
							...
							<ul class="abc">
								<li class="l2">
									<el-upload class="avatar-uploader exampleh" :action="$global.uploadUrl" :show-file-list="false" :on-success="handleHandSuccess" :on-error="handleHandSuccess" :before-upload="beforeAvatarUpload">
			...
			<div style="margin-left:32px">
					...
					<div class="QCellCore" v-if="ali">
						<el-form-item label="优惠套餐：" prop="discounts">
							...
							<el-select v-model="stepThreeForm.discounts" placeholder="请选择" size="mini" @change="discountChange">
								<el-option v-for="(item,index) in discountsList" :key="index" :label="item.concessionName" :value="item">
								</el-option>
							</el-select>
						</el-form-item>
					</div>
				...
				<div style="overflow: hidden" class="addValueTable">
					<ul class="abc">
						<li style="float:unset">
							<el-table ref="addValueTable" :data="objCodeTable" border @selection-change="lalalal" style="width: 100%">
								<el-table-column type="selection" width="55">
								</el-table-column>

								<el-table-column prop="tariffName" label="增值服务名称">
								</el-table-column>

								<el-table-column label='数量' width="250">
									<template slot-scope="scope">
										<el-input-number size="mini" v-model="scope.row.numOfMonth" @change="handleChange(scope.row,scope.$index)" :min="1" label="描述文字">
										</el-input-number>
										<span>{{scope.row.cost}}</span>
										<el-input-number v-if="scope.row.units=='perMonthOne'" size="mini" v-model="scope.row.numOfone" @change="handleChange(scope.row,scope.$index)" :min="1" label="描述文字">
										</el-input-number>
										<span v-if="scope.row.units=='perMonthOne'">{{scope.row.cost2}}</span>
									</template>
								</el-table-column>

								<el-table-column prop="unitsName" label='资费/单位'>
								</el-table-column>

								<el-table-column prop="remarks" label='备注'>
								</el-table-column>

								<el-table-column prop="tariffFee" label='费用'>
								</el-table-column>

								<el-table-column prop="remarks" label='套餐详情'>
								</el-table-column>

								<el-table-column prop="presentsName" label='是否赠送'>
								</el-table-column>
