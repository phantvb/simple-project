				...
				<el-form ref="voiceForm" :model="voiceForm" label-width="90px" class="voiceForm">
					<div class="voiceMsg">
						...
						<el-table :data="tableData" border @row-click="rowInfo" style="width: 100%;margin-bottom:30px;">
							...
							<el-table-column prop="voiceType" label="语音类型" width="180">
							...
							<el-table-column prop="voiceName" label="语音名称" width="180">
								<template slot-scope="scope">
									<div>
										<el-input v-model="scope.row.voicename" @input="voiceNameChange" placeholder="请输入内容" size="mini"></el-input>
									</div>
								</template>
							</el-table-column>
							<el-table-column prop="voiceFile" label="语音文件">
								<template slot-scope="scope">
									<el-upload action="" size="mini" :file-list="scope.row.voicefiles" :on-change="handleChange" :http-request="uploadFile" :before-upload="beforeAvatarUpload" accept=".flv,.mp3,.wma,.swf,.wmv,.mid,.avi,.mpg,.asf,.rm,.rmvb" :limit="1">
