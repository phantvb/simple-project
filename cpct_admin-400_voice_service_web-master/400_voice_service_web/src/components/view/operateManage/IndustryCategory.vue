<template>
    <div id="IndustryCategory">
        ...
        <div v-for="(item,index) in totalCount"
             :key="index"
             class="item">
        ...
        <div>
            <el-dialog
                title="新增/编辑行业类别"
                :visible.sync="addIndustryCategoryDialogVisible"
                width="30%"
                ...
                <span slot="footer" class="dialog-footer">
                    ...
                    <el-button type="primary" @click="submitAdd" size="mini"
                               :style="'display:'+submitData+';'">确 定</el-button>
                    <el-button type="primary" @click="updateMess" size="mini"
    ...
    export default {
        ...
        methods: {
            ...
            submitAdd() {
                ...
                this.$ajax.post('/vos/dic/addDic?dicKey=' + this.addIndustryCategory.key + '&dicValue=' + this.addIndustryCategory.name + '&dicType=industry_category&status=' + status).then((res) => {
            ...
            updateMess() {
                ...
                this.$ajax.post('/vos/dic/editDic?dicValue=' + this.addIndustryCategory.name + '&dicType=industry_category&status=' + status + '&id=' + this.id).then((res) => {
                    if (res.code == 200) {
                        this.$message({
                            type: 'success',
                            message: '编辑成功!'
            ...
            deleteItem(index) {
                this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
                    ...
                    this.$ajax.post('/vos/dic/delDic?id=' + this.tableData[index].id).then((res) => {
                        if (res.code == 200) {
                            this.$message({
                                type: 'success',
                                message: '删除成功!'
